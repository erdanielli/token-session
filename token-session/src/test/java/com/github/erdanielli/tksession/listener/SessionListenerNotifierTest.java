...
class SessionListenerNotifierTest {
  ...
  private SessionListenerNotifier notifier =
      new SessionListenerNotifierBuilder()
  ...
  void shouldNotifyWhenAttributesChange() {
    ...
    verify(httpSessionAttributeListener)
        .attributeAdded(eventExpected(session, "attr", boundValue1));
    verify(boundValue1).valueBound(eventExpected(session, "attr", boundValue1));

    session.setAttribute("attr", boundValue2);
    verify(httpSessionAttributeListener)
        .attributeReplaced(eventExpected(session, "attr", boundValue2));
    ...
    verify(httpSessionAttributeListener)
        .attributeAdded(eventExpected(session, "attr", boundValue3));
    verify(boundValue3).valueBound(eventExpected(session, "attr", boundValue3));

    session.removeAttribute("attr");
    verify(httpSessionAttributeListener)
        .attributeRemoved(eventExpected(session, "attr", boundValue3));
