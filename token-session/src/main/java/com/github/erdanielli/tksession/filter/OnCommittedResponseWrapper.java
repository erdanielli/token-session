...
/**
 * <strong>BASED ON org.springframework.security.web.util.OnCommittedResponseWrapper</strong> from
 * Spring Security<br>
...
abstract class OnCommittedResponseWrapper extends HttpServletResponseWrapper {
  ...
  /**
   * The Content-Length response header. If this is greater than 0, then once {@link
   * #contentWritten} is larger than or equal the response is considered committed.
   */
  private long contentLength;

  /**
   * The size of data written to the response body. The field will only be updated when {@link
   * #disableOnCommitted} is false.
   */
  ...
  private void trackContentLength(String content) {
    if (!this.disableOnCommitted) {
      checkContentLength(content == null ? 4 : content.length());
    }
  }
  ...
  private void checkContentLength(long contentLengthToWrite) {
    ...
    final boolean isBodyFullyWritten =
        this.contentLength > 0 && this.contentWritten >= this.contentLength;
    final int bufferSize = getBufferSize();
    final boolean requiresFlush = bufferSize > 0 && this.contentWritten >= bufferSize;
   ...
   * prior to methods that commit the response. We delegate all methods to the original {@link
   * java.io.PrintWriter} to ensure that the behavior is as close to the original {@link
   * java.io.PrintWriter} as possible. See SEC-2039
   *
   * @author Rob Winch
   */
  private class SaveContextPrintWriter extends PrintWriter {
    ...
    public PrintWriter append(CharSequence csq, int start, int end) {
      checkContentLength((long) (end - start));
   ...
   * methods that commit the response. We delegate all methods to the original {@link
   * javax.servlet.ServletOutputStream} to ensure that the behavior is as close to the original
